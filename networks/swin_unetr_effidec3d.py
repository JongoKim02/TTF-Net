...
from networks.tri_plane_text_fuser import InlineTriPlaneTextFuser
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.fuse5 = InlineTriPlaneTextFuser(in_ch=8 * feature_size, text_dim=text_dim, d=triplane_dim)
        self.fuse4 = InlineTriPlaneTextFuser(in_ch=4 * feature_size, text_dim=text_dim, d=triplane_dim)
        self.fuse3 = InlineTriPlaneTextFuser(in_ch=2 * feature_size, text_dim=text_dim, d=triplane_dim)
        self.fuse2 = InlineTriPlaneTextFuser(in_ch=feature_size, text_dim=text_dim, d=triplane_dim)
    ...
    def forward(self, x_in, sel_text=None):
        # Align sel_text batch size and device
        def _align_sel(sel, B_feat):
            ...
            # Squeeze when sel has shape [B, 1, C]
            if sel.dim() == 3 and sel.size(1) == 1:
                sel = sel.squeeze(1)
            B_txt = sel.shape[0]
            if B_txt == B_feat:
                return sel
