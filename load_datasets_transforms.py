...
def data_loader(args):
    ...
    if args.mode == 'train':
        ...
        # Auto-discover validation data under root
        val_img_dir = os.path.join(root_dir, 'imagesVal')
        val_lbl_dir = os.path.join(root_dir, 'labelsVal')

        test_img = sorted(glob.glob(os.path.join(val_img_dir, '*.nii.gz')))
        test_label = sorted(glob.glob(os.path.join(val_lbl_dir, '*.nii.gz')))

        if len(test_img) == 0 or len(test_label) == 0:
            raise FileNotFoundError(f"[Error] Validation data not found under {root_dir}/(imagesVal, labelsVal)")

        test_samples['images'] = test_img
        test_samples['labels'] = test_label

        print(f'Finished loading all test (val) samples from dataset: {dataset}!')
        print(f'  |- #images: {len(test_img)}')
        print(f'  |- #labels: {len(test_label)}')
        print(f'Number of classes for segmentation: {out_classes}')
    ...
    if dataset == 'feta':
        ...
        train_transforms = Compose(
            [
                ...
                AddChanneld(keys=["image", "label"]),
                Spacingd(keys=["image", "label"], pixdim=(
                    0.5, 0.5, 1.25), mode=("bilinear", "nearest")),
                Orientationd(keys=["image", "label"], axcodes="RAS"),
                ScaleIntensityRanged(
                    keys=["image"], a_min=-125, a_max=275,
                    b_min=0.0, b_max=1.0, clip=True,
        ...
        test_transforms = Compose(
            [
                ...
                AddChanneld(keys=["image"]),
                Spacingd(keys=["image"], pixdim=(
                    0.5, 0.5, 1.25), mode=("bilinear")),
                Orientationd(keys=["image"], axcodes="RAS"),
                ScaleIntensityRanged(
                    keys=["image"], a_min=-125, a_max=275,
                    b_min=0.0, b_max=1.0, clip=True,
                ),
                CropForegroundd(keys=["image"], source_key="label", margin=20),
                # CropForegroundd(keys=["image", "label"], source_key="image"),
                ToTensord(keys=["image"]),
            ]
        )
